...
#include "source_cam.h"			//!< Get GPS data from where
#include "source_imu.h"			//!< Get GPS data from where
/*! @ingroup CLASSES
...
//		Message_Union message_body;					//!< Body of a message//xin:
//		only used in ImportCommando(), SendBoardHealth(), SendTaskHealth()
		SV_Prediction_M sv_predictions[MAX_SV];		//!< Buffer SV predictions

		uint8 packet_body[2048];					//!< Complete packet body
//		uint8 packet_body[8192];					//!< Complete packet body
